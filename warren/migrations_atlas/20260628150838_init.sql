...
CREATE INDEX "agents_class_type_idx" ON "agents" ("class", "type");
...
CREATE TABLE "requests" (
  ...
  PRIMARY KEY ("id"),
  CONSTRAINT "fk-requests-claimed_by" FOREIGN KEY ("claimed_by") REFERENCES "agents" ("id") ON UPDATE NO ACTION ON DELETE NO ACTION
);
-- Create index "requests_inbox_idx" to table: "requests"
CREATE INDEX "requests_inbox_idx" ON "requests" ("target_class", "target_type") WHERE ((status = 1) AND (claimed_by IS NULL));
